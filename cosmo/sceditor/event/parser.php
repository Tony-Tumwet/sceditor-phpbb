...
class parser implements EventSubscriberInterface
{
    ...
    public function initialize_fp($event)
    {
        ...
        $new_color['size']  = array(
            'bbcode_id' => 5,
            'regexp' => array(
                '!\[size=([0-9]+)\](.+)\[/size\]!uise' => "\$this->validate_bbcode_by_extension('\$0', \$this)"
            )
        );
        $new_color['color'] = array(
            'bbcode_id' => 6,
            'regexp' => array(
                '!\[color=(#[0-9a-f]{3}|#[0-9a-f]{6}|[a-z\-]+)\](.+)\[/color\]!uise' => "\$this->validate_bbcode_by_extension('\$0', \$this)"
            )
        );
